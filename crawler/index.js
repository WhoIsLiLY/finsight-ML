...
const processNewsPageWithPuppeteer = async (newsUrl) => {
  ...
  try {
    ...
    const content = await page.evaluate(() => {
      const title = document.querySelector('h1.cover-title')?.innerText.trim() || 'No Title';

      // Untuk video, coba ambil deskripsi/teks
      const paragraphs = Array.from(document.querySelectorAll('p')).map(p => p.innerText.trim()).join(' ');

      const publisherAuthor = document.querySelector('div.byline-attr-author')?.innerText.trim() || 'Unknown';
      const percentageChange = document.querySelector('fin-streamer.percentChange')?.innerText.trim() || 'N/A';
      ...
      const videoDescription = document.querySelector('.video-description')?.innerText.trim() || paragraphs;
...
const processNewsPageWithSelenium = async (newsUrl) => {
  ...
  try {
    ...
    const percentageChange = await safeGetText(By.css('fin-streamer.percentChange'));
    ...
    console.error(`Comprehensive error processing video page (${newsUrl}):`, error);
